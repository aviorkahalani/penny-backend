"use strict";
var __importDefault = (this && this.__importDefault) || function (mod) {
    return (mod && mod.__esModule) ? mod : { "default": mod };
};
Object.defineProperty(exports, "__esModule", { value: true });
const jsonwebtoken_1 = __importDefault(require("jsonwebtoken"));
const user_js_1 = require("../../db/models/user.js");
const AppError_js_1 = require("../../utils/AppError.js");
const http_js_1 = require("../../utils/http.js");
const register = async (credentials) => {
    const user = new user_js_1.User(credentials);
    await user.save();
    const token = _generate_token(user._id);
    const { password: pwd, ...safeUser } = user.toObject();
    return { user: safeUser, token };
};
const login = async (credentials) => {
    const { email, password } = credentials;
    const user = await user_js_1.User.findOne({ email }).select('+password');
    if (!user) {
        throw new AppError_js_1.AppError(http_js_1.NOT_FOUND, 'user not found');
    }
    const isMatchedPassword = await user.comparePasswords(password);
    if (!isMatchedPassword) {
        throw new AppError_js_1.AppError(http_js_1.BAD_REQUEST, 'incorrect password');
    }
    const token = _generate_token(user._id);
    const { password: pwd, ...safeUser } = user.toObject();
    return { user: safeUser, token };
};
const me = async (id) => {
    const user = await user_js_1.User.findById(id);
    if (!user) {
        throw new AppError_js_1.AppError(http_js_1.NOT_FOUND, 'user not found');
    }
    return user;
};
const _generate_token = (userId) => {
    return jsonwebtoken_1.default.sign({ userId }, process.env.JWT_SECRET, { expiresIn: '1h' });
};
exports.default = {
    register,
    login,
    me,
};
